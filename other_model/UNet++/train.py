...
dir_checkpoint = 'checkpoints/Train3'

# val_img = 'data/EndoScene/Val/images/'
# val_mask = 'data/EndoScene/Val/masks/'

def mkdir_time_dir():
    time_now = time.strftime("%Y-%m-%d-%H_%M", time.localtime())
    path ="./vision/kks/{}".format(time_now)
...
def train_net(net,
    ...
    train_data = PolypDataset(image_root=dir_img, gt_root=dir_mask, augmentations=True,transform=True)
    ...
    for epoch in tqdm(range(1,epochs),desc="Epoch",total=epochs):
        ...
        if val_score>val_s:
            ...
            torch.save(net.state_dict(),
                       dir_checkpoint +'/' + f'best_epoch.pth')
