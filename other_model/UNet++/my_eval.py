...
def evaluate(args):
    ...
    headers = args.metrics #['meanDic', 'meanIoU', 'wFm', 'Sm', 'meanEm', 'mae', 'maxEm', 'maxDic', 'maxIoU', 'meanSen', 'maxSen', 'meanSpe', 'maxSpe']
    results = []
    ...
    for dataset in datasets:
        ...
        for i, sample in samples:
            ...
            # # 如果图像的尺寸不一致，使用 resize 调整真实图像（gt_mask）为预测图像（pred_mask）尺寸
            # if pred_mask.shape != gt_mask.shape:
            #     gt_mask = Image.open(os.path.join(gt_root, gt)).resize(pred_mask.shape[1::-1], Image.BILINEAR)
            #     gt_mask = np.array(gt_mask)
            #     if len(gt_mask.shape) == 3:
            #         gt_mask = gt_mask[:, :, 0]  # 只保留第一个通道
...
if __name__ == "__main__":
    ...
    # gt_root: "/home/qyl/dataset/Polyp_dataset/TestDataset"
    # pred_root: "snapshots/UACANet-S"
    # result_path: "results"
    ...
    parser.add_argument('--gt_root', type=str,
                        default="/home/qinyl/qinyilin/Polyp_dataset/TestDataset", help='gt_root')
    parser.add_argument('--pred_root', type=str,
                        default="result_map/UNet++", help='pred_root')
