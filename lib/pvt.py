...
class Decoder(nn.Module):
    def __init__(self, img_size=224,  in_chans=3,  embed_dims=[512, 320, 128, 64],
        ...
        self.Dd1 = Dd(320, 128)
        self.Dd2 = Dd(128, 64)
        # self.Sb0 = Semantbridge(512, 512)
        self.Sb1 = Semantbridge(320, 320)
        self.Sb2 = Semantbridge(128, 128)
        self.Sb3 = Semantbridge(64, 64)
        ...
        self.fuse3 = nn.Sequential(nn.BatchNorm2d(channels*2),
                                   nn.Conv2d(channels*2, 320, kernel_size=3, stride=1, padding=1, bias=False),
                                   nn.GELU(),
                                   dcn2d(320, 320, kernel_size=3, stride=1, padding=1, bias=True))
        self.fuse4 = nn.Sequential(nn.BatchNorm2d(channels*2),
                                   nn.Conv2d(channels*2, 512, kernel_size=3, stride=1, padding=1, bias=False),
                                   nn.GELU(),
                                   dcn2d(512, 512, kernel_size=3, stride=1, padding=1, bias=True))
        self.fm = FrequencyExtract(in_channels = 512)
        # self.initialize()
    def forward(self, pvt):
        ...
        E4 = torch.cat((x4_128, F.interpolate(x5, size=x4.size()[2:], mode='bilinear')), 1)

        E2 = F.relu(self.fuse2(E2), inplace=True)   # 128 44 44
        E3 = F.relu(self.fuse3(E3), inplace=True)   # 128 22 22
        E4 = F.relu(self.fuse4(E4), inplace=True)   # 128 11 11
        
        x_4 = self.fm(x4)  # 512 11 11
        x = self.Dd0(x_4)           # 320 22 22
        x = x * self.Sb1(E3)
        x_3 = x
        x = self.Dd1(x)
        x = x * self.Sb2(E2)
    ...
    def __init__(self, channel=32):
        ...
        for k, v in save_model.items():
            if k.startswith('backbone.'):
                new_key = k.replace('backbone.', '', 1)  # 只替换最前面的
            else:
                new_key = k
            new_state_dict[new_key] = v
        model_dict = self.backbone.state_dict()
        state_dict = {k: v for k, v in new_state_dict.items() if k in model_dict.keys()}
